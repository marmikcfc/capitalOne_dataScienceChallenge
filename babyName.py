...
popularity_outputMale = "The most popular male name is %s, with a total count of %i"
popularity_outputFemale = "The most popular female name is %s, with a total count of %i"
ambiguous_name_output = "The most gender ambiguous name in %s is %s: %d girls and %d boys."
winners_losers_output = (
    "From %s to %s the name with the greatest increase " 
    "in popularity was %s, with a %2.1f%% increase.\n" 
    "The name with the greatest decline in popularity was "
    "%s, with a %2.1f%% change."
)
extrapolated_output = (
    "Inferring from truncated values, from %s to %s the name "
    "with the greatest growth was %s,\nand the name with "
    "the greatest decline was %s."
)
...
# Find most gender ambiguous name in 2013 and 1945
# Ambiguity is calculated by difference in male and female totals, lower is more ambiguous

# Returns a tuple of (year, name, female_count, male_count)
...
def winners_and_losers(year, names_df, extrapolate=False):
    ...
    # We can extrapolate some possible results from truncated data by assuming the most extreme case.
    if extrapolate:
    ...
    # If we're extrapolating from truncated data, percentage changes aren't accurate.
    if extrapolate:
...
def main():
    ...
    print("Done.")
    print(popularity_outputFemale % most_popular_name(state_names,"F"))
    print(popularity_outputMale % most_popular_name(state_names,"M"))
    print(ambiguous_name_output % ambiguous_name_by_year(2013, state_names))
    print(ambiguous_name_output % ambiguous_name_by_year(1945, state_names))
    print(winners_losers_output % winners_and_losers(1980, state_names))
    print(extrapolated_output % winners_and_losers(1980, state_names, extrapolate=True))
